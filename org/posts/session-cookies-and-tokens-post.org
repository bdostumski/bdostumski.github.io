...
1. Session + Cookies: A session ID is stored in a cookie and sent with each request to maintain authentication.
2. Tokens (JWT) + Cookies: JWTs can be stored in cookies (HttpOnly for security) or localStorage/sessionStorage.
3. Tokens vs. Sessions: Sessions require server storage, while tokens (like JWT) allow stateless authentication.
